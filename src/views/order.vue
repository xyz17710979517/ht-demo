...
<template>
  <div>
    ...
    <div class="search_box" style="margin-top: 20px; margin-bottom: 20px">
      <el-row :gutter="20">
        <el-col :span="4">
    ...
    <!-- 表格区域 -->
    <el-table stripe border :data="orderLists">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <a href="#" @click="orderDetail(scope.row.orderNo)">查看</a>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    getorder() {
      this.$http.orderList(1).then((res) => {
        this.orderLists = res.data.data.list;
        this.total = res.data.data.total;
      });
    },
    async handleCurrentChange(val) {
      const { data: res } = await this.$http.orderList(val);
      this.orderLists = res.data.list;
      this.total = res.data.total;
    },
    async search() {
      const { data: res } = await this.$http.searchs(this.input);
      this.orderLists = res.data.list;
      this.total = res.data.total;
    },
    orderDetail(id) {
